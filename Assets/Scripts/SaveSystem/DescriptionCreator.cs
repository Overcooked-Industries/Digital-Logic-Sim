	...
	public static class DescriptionCreator
	{
		...
		public static SubChipDescription CreateSubChipDescription(SubChipInstance subChip)
		{
			...
			(
				...
				(long[])subChip.InternalData?.Clone()
			);
		}
		...
		public static long[] CreateDefaultInstanceData(ChipType type)
		{
			return type switch
			{
				ChipType.Rom_256x32 => new long[256], // ROM contents
				ChipType.Key => new long[] { 'K' }, // Key binding
				ChipType.Pulse => new long[] { 50, 0, 0 }, // Pulse width, ticks remaining, input state old
				ChipType.DisplayLED => new long[] { 0 }, // LED colour
				_ => ChipTypeHelper.IsBusType(type) ? new long[2] : null
			};
		}
