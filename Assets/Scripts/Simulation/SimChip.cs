...
namespace DLS.Simulation
{
	public class SimChip
	{
		...
		public readonly long[] InternalState = Array.Empty<long>();
		...
		public SimChip(ChipDescription desc, int id, long[] internalState, SimChip[] subChips)
		{
			SubChips = subChips;
			ID = id;
			...
			if (ChipType is ChipType.DisplayRGB)
			{
				...
				InternalState = new long[addressSize_8Bit * 2 + 1];
			}
			...
			{
				InternalState = new long[addressSize_8Bit + 1]; // +1 for clock state (to allow edge-trigger behaviour)
			...
			{
				InternalState = new long[internalState.Length];
				UpdateInternalState(internalState);
			}
		}

		public void UpdateInternalState(long[] source) => Array.Copy(source, InternalState, InternalState.Length);
